 ...
 * 
 * @note `bprintf`函数可以输出的字符串最大长度由`constrains.h`中的`PRINTF_STRING_SIZE`宏常量的值决定
 * 
 * @note 
 ...
 * 
 * @warning `bprintf`函数仅供SBI使用, 内核请使用`kprintf`函数, 用户程序请使用`uprintf`函数
 * 
 * @warning 目前`bprintf`最终向`uart`输出字符串
 */
